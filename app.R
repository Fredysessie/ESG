...
Get_Benchmark_list <- function(source = 'Yahoo') {
  ...
  if(source == 'Yahoo'){
    ...
    table <- page %>%
      ...
      html_node(xpath = '//*[@id="nimbus-app"]/section/section/section/article/section[1]/div/div/div/table')%>%
      html_table()
...
server <- function(input, output, session) {
  ...
  output$esg_prices <- renderDT({
    if(is.null(dt_esg_prices())){
      ...
      fullData <- datatable(fullData, escape = FALSE, filter = 'top', rownames = FALSE,
                            extensions = c('ColReorder', 'RowReorder', 'Buttons', 'Responsive'),
                            options = list(scrollY = 280, scrollX = 450, scroller = TRUE,
                                           ...
                                           buttons = list('copy', 'print', list(extend = 'collection',
                                                                                buttons = c('csv', 'excel', 'pdf'),
                                                                                text = 'Download'), I('colvis')),
                                           columnDefs = list(list(targets = "_all", className = "dt-center", width = "50px"))
                            )
      )
      return(fullData)
    }
  ...
  ########## Old version qui fonctionne très bien
  ...
  output$esg_recent_score_DT_full <- renderDT({
    if (is.null(ESG_histo())) {
      ...
      the_hist <- ESG_histo()
      names(the_hist) <- c("Date", "Total ESG Risk Score", "Environment Risk Score", "Social Risk Score", "Governance Risk Score")
      ...
      the_hist <- datatable(the_hist, escape = FALSE, filter = 'top', rownames = FALSE,
                            ...
                            options = list(scrollY = 280, scrollX = 450, scroller = TRUE,
                                           ...
                                           lengthMenu = list(c(10, 50, 100, -1), c('10', '50', '100', 'All')),
                                           pageSizeOptions = c(10, 50, 100),
                                           dom = 'Blfrtip', # Ajouter dom avec B pour les boutons
                                           buttons = list('copy', 'print', list(extend = 'collection',
                                                                                buttons = c('csv', 'excel', 'pdf'),
                                                                                text = 'Download'), I('colvis')),
                                           columnDefs = list(list(targets = "_all", className = "dt-center", width = "50px"))
    ...
    if(!is.null(dim(dt_esg_prices()))){
      ...
      Data <- Data %>%
        tq_transmute(select = Adjusted,
                     ...
                     col_rename = "Return")
      # print("--------")
      # print("after tq_transmute esg price")
      # print(str(Data))
  ...
  #   print(str(Data_return()))
  #
  #   print('head(Data_return())')
  #   print(head(Data_return(), 15))
  #
  #   tidyquant::tq_performance( #ligne 17100
  #     data = Data_return(),
  #     Ra = quote(Return), #Return, #"Return"
  #     performance_fun = VaR,
  #     p = 1 - input$alpha)
  #
  # })

  returns_xts <- reactive({
    req(Data_return())  # s’assure que les données existent
    # print("Data_return()$Return")
    # print(head(Data_return()$Return))

    xts::xts(Data_return()$Return, order.by = Data_return()$Date)
  })

  vaR <- reactive({
    req(returns_xts(), input$alpha)  # pour éviter l'erreur à l'init
    # print('str(Data_return())')
    # print(str(Data_return()))
    ...
    PerformanceAnalytics::VaR(
      R = returns_xts(),
      p = 1 - input$alpha,  # Exemple: alpha = 0.05 → p = 0.95
      method = "historical"
    )

    # ret <- xts::xts(Data_return()$Return, order.by = Data_return()$Date)
    # PerformanceAnalytics::VaR(ret, p = 1 - input$alpha, method = "historical")

  #   Data_return() %>%
  #     dplyr::rename(Ra = Return) %>%
  #     tidyquant::tq_performance(Ra = Ra, performance_fun = VaR, p = 1 - input$alpha)
  ...
  #   # print(str(Data_return()))
  #
  #   # returns_tbl <- dplyr::select(Data_return(), Return)
  #
  #   tidyquant::tq_performance( #ligne 17093
  #
  #     data = Data_return(),
  #     Ra = Return,
  #     # Ra= Return,
  #     # Ra = vars(Return),
  #     # data = dplyr::select(Data_return(), Return),
  #     # Ra = Return,
  #     performance_fun = VaR,
  #     p = 1 - input$alpha)
  #
  #   # Use a different approach to specify the Return column
  #   # data_for_var <- Data_return()
  #   #
  #   # # Option 1: Using select helper functions from tidyquant
  #   # result <- tq_performance(
  #   #   data = data_for_var,
  #   #   Ra = dplyr::select(data_for_var, Return),
  #   #   performance_fun = VaR,
  #   #   p = 1 - input$alpha
  #   # )
  #
  ...
  #   print(str(Data_return()))
  #
  #   # Make sure Data_return() is properly formatted
  #   df <- Data_return()
  #
  #   # Check if it's a valid data frame
  #   if(is.data.frame(df)) {
  #     # Create a proper xts object for time series analysis
  #     data_xts <- xts::xts(df$Return, order.by = df$Date)
  #
  #     # Use PerformanceAnalytics::VaR with the xts object
  #     # Explicitly specify the method parameter
  #     result <- PerformanceAnalytics::VaR(
  #       R = data_xts,
  #       p = 1 - input$alpha,
  #       method = "historical"  # Explicitly specify the method
  #     )
  #
  #     return(result)
  #   } else {
  #     # Return NA if data is not valid
  #     print("Data_return() is not a valid data frame")
  #     return(NA)
  #   }
  # })
  ...
  varRcf <- reactive({
    ...
    z <- zc+1/6*(zc^2-1)*Skewn()[[1]]+1/24*(zc^3-3*zc)*Kurt()[[1]]-1/36*(2*zc^3-5*zc)*Skewn()[[1]]
  ...
  Stats <- reactive({
    ...
    statistiques <- data.frame(Statistic = c("Mean","Volatility","Skewness","Kurtosis", "Minimum","Maximum","Median"), #ligne 17142
                               Value = round(
                                 c(Moyenne()$mean.1,
                                   Volatilite()$sd.1,
                                   # Skewn()$skewness.1, #old
                                   # Skewn()$skewness[1], #previous
                                   Skewn()[[1]],
                                   # Kurt()$kurtosis.1, #old
                                   # Kurt()$kurtosis[1], #previous
                                   Kurt()[[1]],
                                   Min()$min.1,
                                   Max()$max.1,
                                   Mediane()), 4)
  ...
  #
  #   # returns_tbl <- dplyr::select(Data_return(), Return)
  #   returns_tbl <- Data_return()
  #
  #
  #   sp <- returns_tbl %>%
  #     tq_performance(Ra = Return, performance_fun = SharpeRatio, Rf = FR(), p = 0.95)
  #
  #   # sp = tidyquant::tq_performance(
  #   #   data = Data_return(), #ligne 17573
  #   #   Ra = Return,
  #   #   # Ra = Return, #ligne 17565
  #   #   # Ra = vars(Return),
  #   #   # data = dplyr::select(Data_return(), Return),
  #   #   # Ra = Return,
  ...
  #   #   Rf = FR())
  #
  #   # Use a similar approach for Sharpe ratio
  #   # data_for_sharpe <- Data_return()
  #   #
  #   # # Option 1: Using select helper functions
  #   # sp <- tq_performance(
  #   #   data = data_for_sharpe,
  #   #   Ra = dplyr::select(data_for_sharpe, Return),
  #   #   performance_fun = SharpeRatio,
  #   #   Rf = FR(),
  #   #   p = 0.95
  #   # )
  ...
  Sharpe_ratio <- reactive({
    ...
    if (is.null(returns_tbl) || nrow(returns_tbl) == 0) {
      return(NULL)
    }

    # Use the tq_performance with the correct syntax for Ra
    # Note: we're using the magrittr pipe (%>%) and NSE (non-standard evaluation)
    sp <- returns_tbl %>%
      tq_performance(
        Ra = Return,           # No quotes, using NSE
        performance_fun = SharpeRatio,
        Rf = FR(),
        p = 0.95,
        FUN = "StdDev",        # Specify the risk function
  ...
  #     data_xts <- xts::xts(df$Return, order.by = df$Date)
  #
  #     # Use PerformanceAnalytics::SharpeRatio with the xts object
  #     # Explicitly specify all required parameters
  #     sp <- PerformanceAnalytics::SharpeRatio(
  #       R = data_xts,
  #       Rf = FR(),
  #       p = 0.95,
  #       FUN = "StdDev",  # Specify the risk function
  ...
  MSharpe <- reactive({
    ...
    # ts(Data_return()$Return) %>% #ligne 17740
    #   SharpeRatio.modified(FR())

    SharpeRatio(ts(Data_return()$Return),
                Rf = FR(),
                FUN = "StdDev",
                mod = TRUE)
  ...
  expected_shortfall <- reactive({
    req(returns_xts(), input$alpha)
  ...
  output$risks <- DT::renderDT({
    ...
    if (nchar(stock_to_analyze())==0) {
      ...
      dt_ = data.frame(Indicator = c("Value at Risk", "Value at Risk Cornish - Fisher","Expected Shortfall", #ligne 17934

                                     "Max Drawdown", "Semi-variance", "Semi-deviation" ),

                       Value = c(round(c(vaR(), #vaR()$VaR,
                                         varRcf(),
                                         expected_shortfall(), #ES()$ES,
                                         Drawdown_max())
                                       ,4),
                                 round(semivar(),4),
                                 round(MPI2(),4)))
  ...
  output$performances_stock <- DT::renderDT({
    ...
    if (nchar(stock_to_analyze())==0) {
      ...
      if (input$benchmark && nchar(input$Rf_rate)== 0) {
        ...
        # print("MSharpe()")
        # print(MSharpe()) #ligne 17985

        dt_ = data.frame(Indicator = c("Sharpe Ratio", "Adjusted Sharpe Ratio", "Modified Sharpe Ratio", #ligne 17837
